...
async def generate_reply_with_ai(
    ...
    3. 🚨 КРИТИЧЕСКОЕ ПРАВИЛО: Если ИСТОРИЯ ДИАЛОГА НЕ ПУСТАЯ, тебе СТРОГО ЗАПРЕЩЕНО здороваться (никаких "Здравствуйте", "Добрый день" и т.д.). Здоровайся только в самом первом сообщении!
    ...
    В поле "reply" общайся естественно. Если статус "new_booking" или "reschedule", скажи своими словами, что передала информацию архитектору и просишь подождать подтверждения.
    """
...
async def generate_client_summary(history: str, phone_number: str, source_platform: str) -> str:
    """
    Функция-аналитик. Берет историю и делает выжимку. Платформа и номер передаются жестко.
    """
    summary_prompt = f"""
    Проанализируй диалог с клиентом и составь анкету.
    ...
    🎯 *Цель:* [Кратко: суть проекта]
    👨‍👩‍👧‍👦 *Детали:* [Семья / Особенности]
    📍 *Участок:* [Есть/Нет]
    💰 *Бюджет:* [Сумма или "Не обсуждался"]
    📝 *Комментарий Томирис:* [Выжимка пожеланий]
    ...
    try:
        ...
        return "🚨 Ошибка генерации анкеты."